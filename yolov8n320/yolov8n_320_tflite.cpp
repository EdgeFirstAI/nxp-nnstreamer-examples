...
class YOLOv8 {
  ...
  public:
    YOLOv8(const std::string &model, const std::string &image, const std::string &delegate)
        : model(model), image(image), delegate(delegate)
    {
      ...
      this->strPipeline = g_strdup_printf("filesrc location=%s ! jpegdec ! imxvideoconvert_g2d ! imagefreeze ! "
          ...
          "tensor_filter framework=tensorflow-lite model=%s custom=Delegate:External,ExtDelegateLib:%s ! "
      ...
      case 'h':
        std::cout << "Help Options:" << std::endl
                << std::setw(25) << std::left << "  -h, --help"
                  ...
                  << std::setw(25) << std::left << "  -d, --delegate"
                  << std::setw(25) << std::left
                  << "NPU delegate library (default: libneutron_delegate.so)" << std::endl;
        return 1;
...
int main(int argc, char **argv)
{
  std::string model;
  std::string image;
  std::string delegate = "libneutron_delegate.so";
  if (cmdParser(argc, argv, model, image, delegate))
    return 0;
  ...
  YOLOv8 example(model, image, delegate);
  example.run();
